...
// Récupère un choix par son ID, ou retombe sur la valeur par défaut.
...
// Construit le prompt final à partir des sélections du user.
// `selections` est un objet { productType, background, support, lighting }.
// Tout ID inconnu est ignoré (fallback sur le défaut), donc impossible
// d'injecter un prompt arbitraire depuis le client.
...
// Résumé lisible des sélections (pour affichage côté UI).
