...
export default function Success() {
  ...
  useEffect(() => {
    // On attend que le router Next.js ait fini de lire l'URL.
    if (!router.isReady) return;

    // Si pas de session_id dans l'URL, l'user n'est pas passé par Stripe.
    // Plutôt que rester bloqué sur "initialisation", on affiche une erreur claire.
    if (!session_id) {
      setErrorMsg("Cette page n'est accessible qu'après un paiement réussi. Retourne sur l'application pour générer une photo.");
      setStep('error');
      return;
    }
